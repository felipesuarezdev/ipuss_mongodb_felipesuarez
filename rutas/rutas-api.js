...
// Eliminar un trabajo específico de una persona usando $pull
router.delete('/:id/works/:workId', async (req, res) => {
    try {
      // Eliminar el trabajo del array usando $pull
      const result = await Person.findByIdAndUpdate(
        req.params.id,
        { $pull: { works: { _id: req.params.workId } } },
        { new: true }
      );
      ...
      // Enviar la persona actualizada como respuesta
      res.json(result);
    } catch (err) {
